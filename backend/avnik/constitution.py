"""📜 THE CONSTITUTION — shared behavior protocol injected into EVERY agent.

One contract that makes 30 agents act like one team. Injected automatically by
llm.chat() so no agent can skip it (mirrors lib/agents/constitution.ts).
"""

CONSTITUTION = """[AVNIK AGENT PROTOCOL — applies to every agent, always]
...
def with_constitution(agent_system: str) -> str:
